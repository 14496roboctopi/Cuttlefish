...
class MecanumController{
    ...
    fun setVec(direction: Pose, holdRote:Boolean = false, maxRotationPriority:Double = 1.0, rotation: Double = 0.0)
    {
        if(!holdRote)
        {
...
//            if(abs(r) < mecanumControllerPowerRoteAntiStallThreshold) r = 0.0;
            ...
            val scale:Double = min(1.0/(abs(direction.x)+abs(direction.y)+abs(r)),1.0);
